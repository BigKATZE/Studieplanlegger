...
const newArrays = normalizePlannerData({ subjects: [], lectures: [], aiSources: [{ id: 's', subjectId: 'sub', title: 'Kilde', text: ' tekst ', sourceType: 'notes' }], quizAttempts: [{ id: 'a', subjectId: 'sub', question: 'Spm', expectedAnswer: 'Fasit', userAnswer: 'Svar', verdict: 'partial', feedback: 'Nesten' }], workPlans: [{ id: 'p', title: 'Plan', steps: [{ id: 'step', title: 'Gjør' }] }] })
assert.equal(newArrays.aiSources[0].text, 'tekst')
assert.equal(newArrays.quizAttempts.length, 1)
assert.equal(newArrays.workPlans[0].steps[0].completed, false)
assert.equal(isValidBackupData({ subjects: [], lectures: [], aiSources: [] }), true)
assert.equal(isValidBackupData({ subjects: [], lectures: [], aiSources: {} }), false)
