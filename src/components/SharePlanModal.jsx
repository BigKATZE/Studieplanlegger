...
export default function SharePlanModal({ plan, onClose }) {
  ...
  const createLink = async () => {
    ...
    setError('')
    setStatus('')
    const { data, error: invokeError } = await supabase.functions.invoke('shared-plan', {
      body: { action: 'create', planId: plan.id },
    })
    setLoading(false)
    ...
    setShare({
      ...
      url: `${window.location.origin}${window.location.pathname}?plan=${data.token}`,
    })
  ...
  return (
    <Modal title="Del arbeidsplan" onClose={onClose}>
      <p className="text-sm text-muted">
        Mottakeren får en skrivebeskyttet kopi av arbeidsplanen. Lenken utløper etter 30 dager.
      </p>
      {share ? (
        <>
          <input
