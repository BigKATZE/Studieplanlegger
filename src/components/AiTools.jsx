...
export default function AiTools({ data, enabled, onAddReview, onAddSource, onRemoveSource, onAddAttempt, onSaveWorkPlan }) {
  ...
  const evaluateAllQuiz = async () => {
    ...
    try {
      ...
      setError('Kunne ikke vurdere svarene. Prøv igjen.')
    } finally {
      setLoading(false)
    }
  }
          ...
          <ol className="mt-4 space-y-3">
            {result.questions.map((item, index) => (
              <li key={`${item.question}-${index}`} className="rounded-md bg-paper p-4">
                ...
                <details className="mt-2 text-sm"><summary className="cursor-default text-secondary">Vis svar</summary><p className="mt-2 text-muted">{item.answer}</p></details><textarea aria-label={`Ditt svar på ${item.question}`} value={answerInputs[index] || ''} onChange={(event) => setAnswerInputs((value) => ({ ...value, [index]: event.target.value }))} className="mt-3 w-full rounded-md border border-line bg-surface p-2 text-sm" rows="2" placeholder="Skriv ditt svar – la stå tomt for pass" />{feedbackByQuestion[index] && <p className="mt-2 text-sm text-muted">{feedbackByQuestion[index].feedback} ({feedbackByQuestion[index].verdict}{feedbackByQuestion[index].score != null ? `, ${feedbackByQuestion[index].score}/10` : ''})</p>}
