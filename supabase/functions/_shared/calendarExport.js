...
export function exportCalendar(data, options = {}, now = new Date()) {
  const { subjectId = '', from = '', to = '', includeCompleted = false, types = ['lectures', 'assignments', 'exams', 'reviews'], uidNamespace = '' } = options
  ...
  for (const type of new Set(types)) {
    ...
    for (const item of data[type] ?? []) {
      ...
      const lines = ['BEGIN:VEVENT', `UID:${encodeURIComponent(`${uidNamespace ? `${uidNamespace}-` : ''}${type}-${item.id}`)}@studieplanlegger`, `DTSTAMP:${stamp(now)}`]
      if (start) {
