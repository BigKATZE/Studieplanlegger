...
test('uten økt brukes appen som gjest, med Logg inn-knapp', async ({ page }) => {
  ...
  await expect(latestRelease.getByRole('heading', { name: 'v1.4', exact: true })).toBeVisible()
  await expect(latestRelease.getByText('Nyeste versjon', { exact: true })).toBeVisible()
  await expect(latestRelease.getByRole('heading', { name: 'Mobil og navigasjon', exact: true })).toBeVisible()
  await expect(latestRelease.getByRole('heading', { name: 'Samlet eksport', exact: true })).toBeVisible()
  await expect(latestRelease.getByRole('heading', { name: 'Ukebelastning', exact: true })).toBeVisible()
  await expect(latestRelease.getByRole('heading', { name: 'Kalenderabonnement', exact: true })).toBeVisible()
  await expect(page.getByRole('heading', { name: 'v1.3', exact: true })).toBeVisible()
...
test('app loads, pensum tab og fagfilter fungerer', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Oversikt', exact: true }).click()
  await expect(page.getByText('Forelesninger', { exact: true }).first()).toBeVisible()
