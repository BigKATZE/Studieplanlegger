...
test('eksport bevarer kalenderfiltre og lar sikkerhetskopi omgå tom eller ugyldig kalender', async ({ page }) => {
  await seed(page, { ...fixture, subjects: [...fixture.subjects, { id: 'empty', name: 'Økonomi', short: 'Økonomi' }] })
  await expect(page.getByRole('button', { name: 'Importer', exact: true })).toHaveClass(/\bbtn-primary\b/)
  const exporter = page.getByRole('button', { name: 'Eksporter', exact: true })
  await expect(exporter).toHaveClass(/\bbtn-primary\b/)
  await exporter.click()
  const dialog = page.getByRole('dialog', { name: 'Eksporter', exact: true })
  const subject = dialog.getByRole('button', { name: 'Fag', exact: true })
  const calendar = dialog.getByRole('button', { name: 'Kalender', exact: true })
  const backup = dialog.getByRole('button', { name: 'Sikkerhetskopi', exact: true })
  await expect(calendar).toHaveAttribute('aria-pressed', 'true')
  await expect(dialog.getByText('4 hendelser klare for eksport.')).toBeVisible()
  const downloading = page.waitForEvent('download')
  await dialog.getByRole('button', { name: 'Last ned .ics', exact: true }).click()
  await downloading
  await expect(dialog.getByRole('status')).toContainText('Kalenderfilen er klargjort')
  await subject.click()
  await dialog.getByRole('listbox', { name: 'Fag', exact: true }).getByRole('option', { name: 'Økonomi', exact: true }).click()
  await expect(dialog.getByRole('status')).toHaveCount(0)
  await expect(dialog.getByText('0 hendelser klare for eksport.')).toBeVisible()
  await expect(dialog.getByRole('button', { name: 'Last ned .ics', exact: true })).toBeDisabled()
  await backup.click()
  await expect(dialog.getByRole('button', { name: 'Last ned sikkerhetskopi', exact: true })).toBeEnabled()
  await calendar.click()
  await expect(subject).toHaveText('Økonomi')
  await dialog.getByLabel('Fra dato (valgfritt)').fill('2026-09-10')
  await dialog.getByLabel('Til dato (valgfritt)').fill('2026-09-08')
  await dialog.getByRole('checkbox', { name: 'Repetisjoner', exact: true }).uncheck()
  ...
  await calendar.click()
  await expect(subject).toHaveText('Økonomi')
  await expect(dialog.getByLabel('Fra dato (valgfritt)')).toHaveValue('2026-09-10')
  await expect(dialog.getByLabel('Til dato (valgfritt)')).toHaveValue('2026-09-08')
  await expect(dialog.getByRole('checkbox', { name: 'Repetisjoner', exact: true })).not.toBeChecked()
  await expect(dialog.getByRole('checkbox', { name: 'Ta også med fullførte elementer', exact: true })).toBeChecked()
  await expect(dialog.getByRole('alert')).toContainText('gyldig periode')
  await dialog.getByLabel('Fra dato (valgfritt)').fill('2026-09-08')
  await subject.click()
  await dialog.getByRole('listbox', { name: 'Fag', exact: true }).getByRole('option', { name: 'Alle fag', exact: true }).click()
  await expect(subject).toHaveText('Alle fag')
  await expect(dialog.getByText('1 hendelse klar for eksport.')).toBeVisible()
  await expect(dialog.getByRole('button', { name: 'Last ned .ics', exact: true })).toBeEnabled()
  await subject.click()
  await expect(dialog.getByRole('listbox', { name: 'Fag', exact: true })).toBeVisible()
  await page.keyboard.press('Escape')
  await expect(dialog.getByRole('listbox', { name: 'Fag', exact: true })).toHaveCount(0)
...
test('komplett sikkerhetskopi kan importeres med semesterarkivet intakt', async ({ page }) => {
  ...
  await archive(page)
  await page.getByRole('button', { name: 'Eksporter', exact: true }).click()
  let dialog = page.getByRole('dialog', { name: 'Eksporter', exact: true })
  await dialog.getByRole('button', { name: 'Sikkerhetskopi', exact: true }).click()
  const downloading = page.waitForEvent('download')
  await dialog.getByRole('button', { name: 'Last ned sikkerhetskopi', exact: true }).click()
